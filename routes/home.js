	...
	mq_client.make_request('login_queue', msg_payload, function(err, results) {
		...
		if (err) {
			...
			if (results.code == 200) {
				...
				console.log("In Home succes Login and session Email ="
						+ req.session.Email);
				res.send({
					"login" : "Success"
				});
...
exports.addinterest = function(req, res) {
	...
	mq_client.make_request('interest_queue', msg_payload,
...
exports.profile = function(req, res) {
	if (req.session.Email == null) {
		...
		mq_client.make_request('post_queue', msg_payload,
				function(err, results) {
					console.log(results);
					if (err) {
						throw err;
...
exports.interest = function(req, res) {
	if (req.session.Email == null) {
		...
		mq_client.make_request('interest_queue', msg_payload, function(err,
				results) {
			console.log(results);
			if (err) {
				throw err;
...
exports.friendReq = function(req, res) {
	if (req.session.Email == null) {
		...
		mq_client.make_request('friend_queue', msg_payload, function(err,
				results) {
			console.log(results);
			if (err) {
				throw err;
