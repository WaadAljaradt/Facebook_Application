...
exports.CreateGroup = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName, "GroupDesc": GroupDesc, "Email": req.session.Email,"userName":req.session.FirstName, "action":"create"}; 
...
var msg_payload = { "Email": req.session.Email, 'action':"listGroups"}; 
mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
	console.log(results);   
	if(err){    
		throw err;   
...
exports.SearchGroup = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName, "Email": req.session.Email, 'action':"search"}; 
	mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
		console.log(results);   
		if(err){    
			throw err;   
...
exports.deleteG_request = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName, 'action':"deleteGroup"}; 
	mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
		console.log(results);   
		if(err){    
			throw err;   
				...
				if(results.code == 200){     
					...
					res.send({"DeleteGroup":"Success"}); 
					
					}    else {             
						console.log("Invalid delete group");     
						res.send({"DeleteGroup":"Fail"});    
					}   
			}    
	});
...
exports.AddMember = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName, "MemberName" :MemberName, 'action':"add"}; 
	mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
		console.log(results);   
		if(err){    
			throw err;   
				...
				if(results.code == 200){     
					...
					res.send({"addMember":"Success", "value":results.value}); 
					
					}    else {             
						console.log("Invalid Add Member");     
						res.send({"addMember":"Fail","value":results.value});    
					}   
			}    
	});
...
exports.deleteMember = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName, "MemberName" :MemberName, 'action':"deleteMember"}; 
	mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
		console.log(results);   
		if(err){    
			throw err;   
				...
				if(results.code == 200){     
					...
					res.send({"deleteMember":"Success", "value":results.value}); 
					
					}    else {             
						console.log("Invalid Delete Member");     
						res.send({"DeleteMemeber":"Fail","value":results.value});    
					}   
			}    
	});
...
exports.showMembers = function (req,res) { 
	...
	var msg_payload = { "GroupName": GroupName,'action':"show"}; 
	mq_client.make_request('cgroup_queue',msg_payload, function(err,results){     
		console.log(results);   
		if(err){    
			throw err;   
