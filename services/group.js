		...
		mongo.connect(facebookdb, function(){ 
			console.log('Connected to mongo at: ' + facebookdb);
			 var query = {GroupName:msg.GroupName};
			  var groupObject = {GroupName: msg.GroupName, GroupDesc: msg.GroupDesc};
			  var groups_usersObject = {Email: msg.Email, userName : msg.userName,GroupName: msg.GroupName, Role:'1'};
			var coll = mongo.collection('groups'); 
			  coll.findOne(query, function(err, group){
			      if (group) {
			    	  console.log(group);
			    	  console.log("Group already exists Error" +group);
			        res.code = "401";   
					res.value = "Group already exists"; 
					callback(null, res);
			      } else {
			        // create the new user
			        coll.insert(groupObject, function(err,group){
			        	if(!err){
			        	var groupd_userscoll = mongo.collection('groups_users'); 
			        	groupd_userscoll.insert(groups_usersObject, function(err,group){
					     if(!err){	

			        	console.log("success Create of Groupe");
			        	res.code = "200";   
		...
		mongo.connect(facebookdb, function(){ 
			...
			coll.findOne(query, 
				...
				if (group) { 
					...
					var groups_userquery = {Email :msg.Email, GroupName :msg.GroupName};
					groups_userscoll.findOne(groups_userquery, function(err, group_user){
...
function AddMember(msg, callback){   
			...
			console.log("In Add member handle request:"+ msg.MemberName); 
				...
				mongo.connect(facebookdb, function(){ 
					console.log('Connected to mongo at: ' + facebookdb);
					var Userquery = {FirstName :msg.MemberName};
					
					//var query = {GroupName:msg.GroupName, Email:msg.Email};
					//  var groupObject = {Email: msg.Email, GroupName: msg.GroupName, GroupDesc: msg.GroupDesc,Role:msg.Role};
					var userscoll = mongo.collection('users'); 
					userscoll.findOne(Userquery, function(err, user){
					      if (user) {				
							var groups_userquery = {GroupName:msg.GroupName, Email:user.Email};
							console.log(" Member  Exsit in Users ");
							var groups_userscoll = mongo.collection('groups_users');
							groups_userscoll.findOne(groups_userquery, function(err, group){
							 if (group) {
					        ...
					        // Insert add Member
					    	var groups_usersObject = {Email: user.Email, GroupName: msg.GroupName,Role:'0',userName:user.FirstName};
					    	groups_userscoll.insert(groups_usersObject, function(err,group){
					        	if(err){
					        		console.log("error");
					        		res.code = "401";   
					        	...
					        	console.log("success Add Member to Group" );
					        	res.code = "200";   
					        	...
					        	console.log(" Error :User Doesnt Exsit" );
					        	res.code = "401";   
...
function deleteMember(msg, callback){   
							...
							console.log("In Delete Member handle request:"+ msg.MemberName); 
								mongo.connect(facebookdb, function(){ 
									console.log('Connected to mongo at: ' + facebookdb);
									 var query = {FirstName:msg.MemberName};
									...
									coll.findOne(query, 
										...
										if (err || user == null ) { 
											...
											var query = {GroupName:msg.GroupName, Email:user.Email};
											var groups_usercoll = mongo.collection('groups_users');
											groups_usercoll.findOne(query, 
											...
											groups_usercoll.remove(query, 
												...
												if (err ) { 
													console.log("Error Delete Member  In groups_users ");
													res.code = "401";   
													res.value = "Error";
													callback(null, res);
											...
											console.log("Succes Delete Member");
											res.code = "200";   
