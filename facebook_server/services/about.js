...
function about(msg, callback) {
	...
	mongo.connect(facebookdb, function() {
		...
		ecoll.find(query).toArray(function(err, education) {
			console.log("education");
			console.log(education);
			var wcoll = mongo.collection('work');

			wcoll.find(query).toArray(function(err, work) {
				console.log("work");
				console.log(work);

				var pcoll = mongo.collection('place');

				pcoll.find(query).toArray(function(err, place) {
					console.log("place");
					console.log(place);

					var ccoll = mongo.collection('contactInfo');

					ccoll.findOne(query, function(err, contactInfo) {
						console.log("contactInfo");
						console.log(contactInfo);

						var evcoll = mongo.collection('event');

						evcoll.find(query).toArray(function(err, event) {
							console.log("event");
							console.log(event);
	...
	mongo.connect(facebookdb, function() {
		...
		ccoll.findOne(query, function(err, contactInfo) {
			if (contactInfo) {//update
				...
				if (msg.phoneNumber)
					objUpdate.phoneNumber = msg.phoneNumber;
				if (msg.currentCity)
					objUpdate.currentCity = msg.currentCity;
				if (msg.dateOfBirth)
					objUpdate.dateOfBirth = msg.dateOfBirth;
				if (msg.emailc)
					objUpdate.emailc = msg.emailc;
				var update = {
					$set : objUpdate
				};
...
function addevent(msg, callback) {
	...
	mongo.connect(facebookdb, function() {
		...
		var eventObj = {
			...
			eventdate : msg.eventdate,
			eventPlace : msg.eventPlace,
			evenDisc : msg.evenDisc,
